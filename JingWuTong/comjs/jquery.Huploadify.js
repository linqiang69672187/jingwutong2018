...
$.fn.Huploadify = function(opts){
	...
	this.each(function(){
		...
		inputStr += '"/>';
		inputStr += '<button id="file_upload_' + instanceNumber + '-button" href="javascript:void(0)" class="uploadify-button">';
		inputStr += option.buttonText;
		inputStr += '</button>';
		var uploadFileListStr = '<div id="file_upload_'+instanceNumber+'-queue" class="uploadify-queue" style="display:inline"></div>';
				  	...
				  	var thisFile = files[i];
				  		if(parseInt(formatFileSize(thisFile.size,true))>option.fileSizeLimit){
				  		   // alert('文件' + thisFile.name + '大小超出限制！');
				  		    $("#drtext span").text('文件' + thisFile.name + '大小超出限制！');
				  			continue;
				  		}
						if($.inArray(thisFile.name.split('.').pop(),typeArray)>=0){
						    ...
						    $("#drtext span").text('文件' + thisFile.name + '类型不允许！');
